...
namespace fullsail_ai { namespace algorithms {
	...
	class PathSearch
	{
		struct Edge
		{
			...
			Edge(SearchNode * _node, float _cost);
			~Edge();
		};
	...
	public:
		...
		{
			//Node* parent;
			Edge::SearchNode* vertex;
			PlannerNode * Parent;

			double HeuristicCost;
			float GivenCost;
			float Cost;
			//PlannerNode(SearchNode* _node) : vertex(_node)
			//{}
		};
	...
	private:
		...
		TileMap * MyTileMap;
		Tile * GoalTile;
		std::map<Tile*, Edge::SearchNode*> MyMapGraph;
		...
		std::unordered_map<Edge::SearchNode*, PlannerNode*> Visited;
		//std::map<SearchNode*, PlannerNode*> visited;
	public:

		bool CheckIfAdjacent(Tile * CurrentTile, Tile * AdjacentTile);
